...
from models.xbert import BertConfig, BertForMaskedLM, BertModel, BertForTokenClassification
...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    # print('use_mlm_loss##############################################################################################################################################################################################################################################################################################################################################:',use_mlm_loss)
    if use_mlm_loss:
        ...
        if 'roberta' in config['text_encoder']:
            ...
            text_encoder= BertForTokenClassification(config=config_text, label_smoothing=config['label_smoothing'])
...
class XVLMBase(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False, load_cross_params=False,
        ...
        self.vision_encoder_f = self.build_vision_encoder(config, load_params=load_vision_params)
        ...
        self.norm_layer_aggr =nn.LayerNorm(self.text_width)
        self.cls_token_local = nn.Parameter(torch.zeros(1, 1, self.text_width))
        self.cls_token_local_e = nn.Parameter(torch.zeros(1, 1, self.text_width))
        self.cls_token_local_e_2 = nn.Parameter(torch.zeros(1, 1, self.text_width))

        self.cls_token_local_L = nn.Parameter(torch.zeros(1, 1, self.text_width))
        self.cls_token_local_L_2 = nn.Parameter(torch.zeros(1, 1, self.text_width))

        self.aggregator = nn.MultiheadAttention(self.text_width, 12, dropout=0.0, batch_first=True)

        self.norm_layer_it_cross_atten =nn.LayerNorm(self.text_width)
        self.it_cross_attn = nn.MultiheadAttention(self.text_width, 12, dropout=0.0, batch_first=True)
    ...
    def get_image_embeds_f(self, image, image_atts=None, idx_to_group_img=None, output_hidden_states=None, output_attentions=None):
        assert image.dim() == 4
        assert output_hidden_states == output_attentions

        if idx_to_group_img is None:
            image_embeds = self.vision_encoder_f(image, output_hidden_states=output_hidden_states, output_attentions=output_attentions)
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
            return image_embeds, image_atts  # full attention
            ...
            if image_atts is None:
                image_embeds_fullatts = self.vision_encoder_f(image)
                ...
                assert image_atts.size(0) == idx_to_group_img.size(0)  # bsz
                image_embeds, image_embeds_fullatts = \
                    self.vision_encoder_f(image, idx_to_group_img=idx_to_group_img, image_atts=image_atts)
    ...
    def get_image_embeds_m(self, image, image_atts=None, idx_to_group_img=None, output_hidden_states=None, output_attentions=None):
        ...
        if idx_to_group_img is None:
            image_embeds_m = self.vision_encoder_m(image, output_hidden_states=output_hidden_states, output_attentions=output_attentions)
            image_atts = torch.ones(image_embeds_m.size()[:-1], dtype=torch.long).to(image.device)
            return image_embeds_m, image_atts  # full attention
            ...
            if image_atts is None:
                ...
                assert image_atts.size(0) == idx_to_group_img.size(0)  # bsz
                image_embeds_m, image_embeds_fullatts = \
                ...
                return image_embeds_m, image_atts, image_embeds_fullatts
    ...
    def get_vision_embeds_f(self, image, image_atts=None, idx_to_group_img=None, output_hidden_states=None, output_attentions=None):
        """
        vision_embeds: cls + patch embeds
        """
        assert output_hidden_states == output_attentions
        ...
        assert image.dim() == 4
        return self.get_image_embeds_f(image, image_atts=image_atts, idx_to_group_img=idx_to_group_img,
                                     output_hidden_states=output_hidden_states, output_attentions=output_attentions)
    ...
    def get_text_embeds(self, text_ids, text_atts, output_hidden_states=None, output_attentions=None):
        ...
        # print("Model embedding size:", self.text_encoder.bert.embeddings.word_embeddings.num_embeddings)

        # print("Max input_id:", text_ids.max())  # 检查最大索引值
        # print("Min input_id:", text_ids.min())  # 检查最小索引值
        # print(text_ids.device)
        # print(text_atts.device)
    ...
    def get_text_embeds_m(self, text_ids, text_atts, output_hidden_states=None, output_attentions=None):
        ...
        encoder = self.text_encoder_m.bert if hasattr(self.text_encoder, 'bert') else self.text_encoder_m
    ...
    def get_cross_embeds_ro(self, image_embeds, image_atts, text_ids=None, text_embeds=None, text_atts=None, output_hidden_states=None, output_attentions=None):
    ...
    def FLM(self, image, image_embeds, text_embeds, text_atts):
        bs = image.size(0)
        cls_tokens_local = self.cls_token_local.expand(bs, -1, -1)

        text_atts_mask_clone = text_atts.clone()
        local_feat_padding_mask_text = text_atts_mask_clone == 0
        cross_att = self.it_cross_attn(query=self.norm_layer_it_cross_atten(image_embeds),
        ...
        local_feat_it_cross_attn = image_embeds + cross_att
    ...
    def get_mac_loss(self, image, image_embeds, text_embeds, real_label_pos,idx_to_group_img=None, text_ids=None, text_atts=None, temp=None, alpha=0.5):
        image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image.device)
        ...
        with torch.no_grad():
            ...
            image_embeds_m,_ = self.get_vision_embeds_m(image, image_atts=image_atts, idx_to_group_img=idx_to_group_img)
              
            

            image_feat_m = self.get_features(image_embeds=image_embeds_m)
            image_feat_all = torch.cat([image_feat_m.t(),self.image_queue.clone().detach()],dim=1)
            
            text_output_m = self.get_text_embeds_m(text_ids, text_atts) #already last_hidden_state
        ...
        loss_ITM = L_itm(image_feat, text_feat)

        self._dequeue_and_enqueue(image_feat_m, text_feat_m)

        return loss_MAC,loss_ITM
