...
from tools.multilabel_metrics_for_DGM4 import AveragePrecisionMeter, get_multi_label_DGM4
from tools.multilabel_metrics_for_SAMM import  get_multi_label_SAMM


from models.PMMD_2 import PMMD_2 
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, label, text, fake_image_box, fake_word_pos, W, H, pred_pos,image_path) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        logits_real_fake, logits_multicls, output_coord, logits_tok = model(image, label, text_input, fake_image_box, fake_token_pos, pred_pos, image_path, W, H, is_train=False)
        ...
        # ----- multi metrics -----
        target, _ = get_multi_label_DGM4(label, image)
...
def main_worker(gpu, args, config):
    ...
    model = PMMD_2(args=args, config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, init_deit=True)
